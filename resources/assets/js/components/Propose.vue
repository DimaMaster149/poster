<template>
  <div class="container-block">
      ...
      <div class="propose">
        <div class="propose-form">
          <form @submit="proposeStore" enctype="multipart/form-data">
            ...
            <div
              class="propose-form__image-description"
    ...
    <div class="events__wrap">
      <div class="events">
        <div class="events__title">Ваши предложенные события:</div>
...
export default {
  ...
  computed: {
    proposedUserEvents: function() {
      console.log(
        "computed propose",
        this.$store.getters.getProposedUserEvents
      );
  ...
  methods: {
    ...
    proposeStore(e) {
      ...
      this.axios
        .post("/events/proposed/store", formData, config)
        .then(function(response) {
          ...
          swal({
            title: "Добавлено!",
            text:
              "Спасибо за предолженное событие. Оно будет рассмотрено администратором. ",
            icon: "success"
          });
        })
        .catch(function(error) {
          swal({
            title: "Ошибка",
            text: "Ваше событие не добавлено. Проверьте данные еще раз.",
            icon: "warning"
          });
          console.log(error);
        });
    ...
    deleteEvent(event) {
      ...
      swal({
        ...
        if (willDelete) {
          this.axios
            .delete("/events/proposed/delete/" + event.event_id)
            .then(function(resp) {
