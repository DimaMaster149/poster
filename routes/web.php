...
Route::get('/', function () {
	if(Auth::check()) {
		if(User::find(Auth::id())->user_is_admin == 1) {
			$userType = 1;
		} else{
			$userType = 0;
		}
		return view('mainPage')->with('userType', $userType);
	} else {
		return view('mainPage')->with('userType', 2);
	}
});
...
Route::get('/events/published/{date?}', 'User\EventController@publishedEventsSearch');
...
//Route::post('/admin/events/published/{id?}', 'Admin\AdminEventController@publishedAdminEventsUpdate');
	...
	if(Auth::check()) {
		$userType = User::find(Auth::id())->first()->user_is_admin;
		return view('mainPage')->with('userType', $userType);
	} else {
		return view('mainPage')->with('userType', 2);
	}
